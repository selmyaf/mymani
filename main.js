<script>
  ...
  let pemasukanBulanan = 0; // Contoh nilai pemasukan bulanan
    let budget = { konsumsi: 1100000, internet: 50000, rekreasi: 150000 };
    let daftarPemasukan = [];
    ...
    function tampilkanTabelPemasukan() {
        ...
        tabelHTML += '</table>';
        document.getElementById('tabelPemasukan').innerHTML = tabelHTML;
    }
    ...
    function tampilkanTabelPengeluaran() {
        ...
        tabelHTML += '</table>';
        document.getElementById('tabelPengeluaran').innerHTML = tabelHTML;
    }
    ...
    function tampilkanTabelSisaBudget() {
        let tabelHTML = '<h2>Tabel Sisa Persen Budget</h2><table border="1"><tr><th>Kategori</th><th>Sisa Persen Budget</th></tr>';
        Object.keys(budget).forEach(kategori => {
            let sisaBudget = budget[kategori];
            let sisaPersenBudget = ((sisaBudget) / (budget[kategori] + daftarPengeluaran.filter(p => p.kategori === kategori).reduce((total, p) => total + p.nominal, 0))) * 100;
    ...
    function hitungSisaUang() {
        ...
        let sisaUangHTML = `<div class="row">
                <div class="col-md-6 grid-margin stretch-card">
                  <div class="card card-tale" >
                    
                <h3 class ='mb-4'>Pemasukan</h3> <p class="fs-30 mb-2" >Rp ${sisaUang}</p>`;
        document.getElementById('sisaUang').innerHTML = sisaUangHTML;

        if (sisaUang <= 0) {
            document.getElementById('sisaUang').style.color = 'red';
        } else {
            document.getElementById('sisaUang').style.color = 'black';
        }
    }
